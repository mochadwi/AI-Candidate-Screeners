...
router.post('/', async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const projectFileInfo = await fileService.validateFileExists(projectFileId, 'project');
    ...
    processEvaluationAsync(job.id, cvFileInfo.path, projectFileInfo.path, title);
...
router.delete('/:jobId', async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    if (job.status === 'processing') {
      throw new AppError('Cannot cancel job that is currently processing', 400);
    }
